...
while (<READ>) {
	...
	$row{'drug_class'} = $drugDic{$mas[5]};
	$row{'drug'} = $mas[5];
	$row{'disease'} = $mas[1];
	$row{'type'} = $mas[4];
	$row{'alteration'} = $mas[3];
	$row{'LoE'} = uc($mas[6]);
...
foreach my $patient (keys %struct) {
	...
	foreach my $arg (@{$struct{$patient}}) {
		my $ass = $arg->{'gene'}.'-'.$arg->{'alteration'}.'-'.$arg->{'drug'}.'-'.$arg->{'disease'};
#		next if $arg->{'LoE'} eq '1';
#		next if $arg->{'LoE'} eq 'R1';
#		next if $arg->{'LoE'} eq 'R2';
		next if $known{$ass};
		$known{$ass} = 1;
		push @rec, [$arg->{'gene'}, $arg->{'alteration'}, $arg->{'drug'}, $arg->{'disease'}];
		}
	foreach my $arg (@rec){
		my $LoE = 5;
		foreach my $el (@{$struct{$patient}}) {
			next unless $el->{'gene'} eq $arg->[0];
			next unless $el->{'alteration'} eq $arg->[1];
